...
function validateLinuxLibcOverride(): void {
  ...
  if (process.platform === "android" || libc === undefined || libc === "" || libc === "glibc" || libc === "musl") return
...
async function resolveNativePackage() {
  ...
  if (process.platform === "linux" || process.platform === "android") {
    ...
    if (process.arch === "x64") {
      if (process.platform === "android" || process.env.OPENTUI_LIBC === "musl") {
...
if (isBunfsPath(targetLibPath)) {
  ...
  if (!process.env.TMPDIR) {
    throw new Error("TMPDIR must be configured before materializing OpenTUI runtime assets")
  }
  const materializedPath = join(process.env.TMPDIR, basename(targetLibPath))
  if (!existsSync(materializedPath)) {
    writeFileSync(materializedPath, new Uint8Array(await Bun.file(targetLibPath).arrayBuffer()))
  }
  targetLibPath = materializedPath
}
