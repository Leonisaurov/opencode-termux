...
fn isMacOSSDKPath(path: []const u8) bool {
    const trimmed_path = blk: {
        var end = path.len;
        while (end > 0 and (path[end - 1] == '/' or path[end - 1] == '\\')) {
            end -= 1;
        }
        break :blk path[0..end];
    };
...
fn resolveMacOSSDKPath(b: *std.Build) ?[]const u8 {
    ...
    for (env_vars) |env_var| {
        if (std.process.getEnvVarOwned(b.allocator, env_var)) |sdk_path| {
...
fn addNativeAudioDependencies(
    ...
    switch (target.result.os.tag) {
        ...
        .linux => {
            // Android: dl y pthread están fusionados en libc.so
...
fn checkZigVersion() void {
    ...
    inline for (SUPPORTED_ZIG_VERSIONS) |supported| {
        if (current.major == supported.major and
            current.minor == supported.minor and
            current.patch == supported.patch)
        {
            is_supported = true;
        }
    }
    if (!is_supported) {
        std.debug.print("Error: Unsupported Zig version {}.{}.{}\n", .{
