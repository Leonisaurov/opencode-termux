...
pub const BufferedBackend = struct {
    ...
    fn bufferWrite(ctx: WriterCtx, data: []const u8) error{BufferFull}!usize {
        ...
        // OTUI DEBUG (temporal): estado del Writer en cada flush — bufferLen.* = w.end,
        // buffer.*.len = w.buffer.len. Si bufferLen.* es gigante/corrupto, confirma heap
        // overflow externo al renderer (hipótesis a).
        if (std.posix.getenv("OTUI_DEBUG_LEN") != null) {
            std.debug.print("OTUI_BUF bufferLen={d} buffer.len={d} data.len={d}\n", .{ bufferLen.*, buffer.*.len, data.len });
        }
