...
export KILO_SRC="${KILO_SRC:-${BUILD_DIR:-${WORK_DIR}}/kilocode-src-latest}"
...
KILO_OPENTUI_SRC="${KILO_OPENTUI_SRC:-${WORK_DIR}/opentui-src-kilo}"
KILO_OPENTUI_TARGET="aarch64-linux-android.24"
# libc bionic del NDK para zig (mismo archivo que usa opencode_build.sh)
ZIG_LIBC_FILE="${ZIG_LIBC_FILE:-$REPO_ROOT/build/libc-android.txt}"
ZIG_BIN="${ZIG_BIN:-zig}"

OUTPUT="${KILO_OUTFILE:-${REPO_ROOT}/kilo-android}"
...
incremental_exec kilo \
    --input kilocode_build.sh --input scripts/env.sh \
    --input scripts/build-kilo-android.ts --input "$KILO_SRC" \
...
if [ ! -f "$MARKERS/kilo-deps" ]; then
    ...
    if [ ! -f "$MARKERS/opentui-kilo-built" ]; then
        ...
        export ZIG_LOCAL_CACHE_DIR="$TMPDIR/zig-cache-kilo-$$"
        export ZIG_GLOBAL_CACHE_DIR="$TMPDIR/zig-global-kilo-$$"
...
KILO_OUTFILE="$OUTPUT" KILO_SRC="$KILO_SRC" KILO_VERSION="$KILO_VERSION" KILO_MINIFY="${KILO_MINIFY:-1}" \
    ...
    tcr "$ANDROID_BUN" run scripts/build-kilo-android.ts 2>&1
