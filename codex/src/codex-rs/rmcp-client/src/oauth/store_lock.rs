...
impl OAuthStoreLock {
    ...
    fn acquire_in_with_mode(
        ...
        loop {
            // CODEX-TERMUX-ANDROID-PATCH: std::fs::File::try_lock/try_lock_shared is
            // unsupported on Android/bionic (ErrorKind::Unsupported); single-user
            // env → always acquired.
            ...
            let result: std::result::Result<(), std::fs::TryLockError> = Ok(());
            match result {
