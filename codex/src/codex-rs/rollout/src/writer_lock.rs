...
impl WriterLockCoordinator {
    ...
    pub fn acquire(self: &Arc<Self>, thread_id: ThreadId) -> io::Result<WriterLockGuard> {
        ...
        // Android/bionic (ErrorKind::Unsupported) with the pinned Rust toolchain;
        // single-user env → treat the writer lock as always acquired.
    ...
    pub(crate) fn try_acquire_for_publication(
        ...
        match OpenOptions::new().read(true).write(true).open(path) {
            Ok(file) => {
                // CODEX-TERMUX-ANDROID-PATCH: std::fs::File::try_lock is unsupported
                // on Android/bionic (ErrorKind::Unsupported); single-user env →
                // publication probing always succeeds.
                #[cfg(not(target_os = "android"))]
                let lock_result = file.try_lock();
                #[cfg(target_os = "android")]
                let lock_result = Ok(());
    ...
    fn lock_coordination(&self) -> io::Result<File> {
        ...
        // Android/bionic (ErrorKind::Unsupported); single-user env → skip the flock.
        #[cfg(not(target_os = "android"))]
    ...
    fn remove_stale_thread_locks(&self) -> io::Result<()> {
        for entry in fs::read_dir(&self.directory)? {
            ...
            // CODEX-TERMUX-ANDROID-PATCH: try_lock is unsupported on Android/bionic →
            // treat stale-lock inspection as "acquired" (cleanup still works).
            #[cfg(not(target_os = "android"))]
            let lock_result = file.try_lock();
            #[cfg(target_os = "android")]
            let lock_result = Ok(());
